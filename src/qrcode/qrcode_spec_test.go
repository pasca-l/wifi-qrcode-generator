...
func TestEncodeSrc(t *testing.T) {
	testcases := []struct {
		src     string
		mode    EncodeMode
		version Version
	...
	for _, tt := range testcases {
		t.Run("testing EncodeSrc()", func(t *testing.T) {
			spec := QRCodeSpec{
				mode:    tt.mode,
				version: tt.version,
				ecl:     tt.ecl,
			...
			if err != nil && err.Error() != tt.wantErr.Error() {
				t.Errorf("EncodeSrc() error = '%v'; expected '%v'", err, tt.wantErr)
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("EncodeSrc() = %v; expected %v", got, tt.want)
			}
		})
	}
}

func TestApplyErrorCorrection(t *testing.T) {
	testcases := []struct {
		msg     utils.Bytes
		mode    EncodeMode
		version Version
	...
	for _, tt := range testcases {
		t.Run("testing ApplyErrorCorrection()", func(t *testing.T) {
			spec := QRCodeSpec{
				mode:    tt.mode,
				version: tt.version,
				ecl:     tt.ecl,
			}
			got, err := ApplyErrorCorrection(tt.msg, spec)
			if err != nil && err.Error() != tt.wantErr.Error() {
				t.Errorf("ApplyErrorCorrection() error = '%v'; expected '%v'", err, tt.wantErr)
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ApplyErrorCorrection() = %v; expected %v", got, tt.want)
			}
		})
	}
}
